...
public class RAF {
    ...
    public void actualizar(Registro t) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            long pos = 8 + i * SIZE;
            raf.seek(pos);
            String codigo_actualizar = raf.readUTF().replace('\0',' ').trim();
            //System.out.println(codigo_actualizar);
            if (t.getCodigo().equals(codigo_actualizar) ) {
    ...
    public void borrar(String identificador) throws IOException {
        ...
        for(int i=0;i<n;i++){
            ...
            if(!(t.getCodigo().equals(identificador))){                
